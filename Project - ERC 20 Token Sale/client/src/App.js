...
class App extends Component {
  ...
  handleBuyTokens = async () => {
    // buy tokens for this.accounts[0]
  ...
  handleKycSubmit = async () => {
    ...
    await this.kycContract.methods.setKycCompleted(kycAddress).send({from: this.accounts[0]});
              // pravime GET na kycContract , i od nego ja zemame funkcijata setKycCompleted
  ...
  render() {
    ...
    return (
      <div className="App">
        ...
        <p>You currently have: {this.state.userTokens} Tokens</p>
        <button type="button" onClick={this.handleBuyTokens}>Buy more tokens</button>      </div>
    );
  }
}
